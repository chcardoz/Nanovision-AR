...
public class MagnifyingGlass : MonoBehaviour
{
	...
	float[] m_Amount =  new float[8];
	float[] m_RadiusX = new float[8];
	float[] m_RadiusY = new float[8];
	float[] m_RadiusInner = new float[8];
	float[] m_RadiusOuter = new float[8];
	float m_MouseX = 0f;
	float m_MouseY = 0f;
	bool m_TraceMouse = false;
	Rect[] m_GUIRects = new Rect[9];
	bool m_UseComplicated = false;
	bool m_UseMultiple = false;
	bool m_InvertScale = false;
	int m_GlassIndex = 0;
